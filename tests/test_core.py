...
from app.services.auth import hash_password, normalize_username, session_token_hash, verify_password

def test_shared_login_password_is_hashed():
    assert normalize_username("  Cyber Expert ") == "cyber expert"
    encoded=hash_password("test-only-password")
    assert encoded.startswith("pbkdf2_sha256$600000$")
    assert "test-only-password" not in encoded

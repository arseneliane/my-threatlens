...
For DeepSeek chat on every laptop, create an Ollama API key and set `OLLAMA_API_KEY` as a secret in Render. The Blueprint sets `OLLAMA_URL=https://ollama.com` and `OLLAMA_MODEL=deepseek-v4-pro`. Both the site-wide and finding-specific assistants call Ollama from the server, so the browser never receives the key and does not need Ollama installed.
